...
public class ChemUtils {
	...
	public static enum FingerprintsType {
		...
		ADDITIVE_CIRCULAR_4_1024_NO_SCALE, ADDITIVE_CIRCULAR_4_4096_NO_SCALE, ADDITIVE_CIRCULAR_6_1024_NO_SCALE,
		CIRCULAR_4_512;
	}
	...
	public static float[] fingerprints(String smiles, FingerprintsType type) throws CDKException {
		...
		BitSet fp = null;
		if (type == FingerprintsType.ADDITIVE_CIRCULAR_4_4096_NO_SCALE) {
			return (circularAdditiveFingerPrints(mol, CircularFingerprinter.CLASS_FCFP4, 4096, false));
		}
		if (type == FingerprintsType.ADDITIVE_CIRCULAR_4_1024_NO_SCALE) {
			return (circularAdditiveFingerPrints(mol, CircularFingerprinter.CLASS_FCFP4, 1024, false));
		}
		if (type == FingerprintsType.ADDITIVE_CIRCULAR_6_1024_NO_SCALE) {
			return (circularAdditiveFingerPrints(mol, CircularFingerprinter.CLASS_FCFP6, 1024, false));
		}
	...
	public static void main(String[]args) throws CDKException {
		float[] fp = fingerprints("C",FingerprintsType.MACCS);
		System.out.println(fp.length);
		for(int i=0;i<fp.length;i++) {
			if(fp[i]!=0) {
				System.out.println(i+": "+fp[i]+" | ");
			}
		}
		
		fp = fingerprints("CC",FingerprintsType.MACCS);
		System.out.println(fp.length);
		for(int i=0;i<fp.length;i++) {
			if(fp[i]!=0) {
				System.out.println(i+": "+fp[i]+" | ");
		...
		fp = fingerprints("CCC",FingerprintsType.MACCS);
		System.out.println(fp.length);
		for(int i=0;i<fp.length;i++) {
			if(fp[i]!=0) {
				System.out.println(i+": "+fp[i]+" | ");
		...
		fp = fingerprints("CCCCCCCCCCCCCCCCCCCCCCCCC",FingerprintsType.MACCS);
		System.out.println(fp.length);
		for(int i=0;i<fp.length;i++) {
			if(fp[i]!=0) {
				System.out.println(i+": "+fp[i]+" | ");
		...
		fp = fingerprints("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC",FingerprintsType.MACCS);
		System.out.println(fp.length);
		for(int i=0;i<fp.length;i++) {
			if(fp[i]!=0) {
				System.out.println(i+": "+fp[i]+" | ");
